...
if (atype) { # Type A data analysis
  for (i in 1:8) {   #Runs through each trace to collect voltage and dFF data
    max_ave <- -1
    tempvolt <- -100
    volt <- -300
    j <- stime + 1000
  ...
  for (i in 1:8) {   #Runs through each trace to collect voltage and dFF data at end of pulse
    volt <- mean(raw_data[(endtime-510):(endtime-10), 2*i+i])
    max_ave <- mean(tracelist[[i]][(endtime-500):endtime, 2])
    trace_data[i, 1] <- volt
...
for (j in 2:8) { # Runs through each trace and adds it to the plot
...
if(vresp=="y") { # If you're crazy enough to want Voltage vs time data, this block of code plots it
  ...
  for (j in 2:8) { # Each trace is added to the plot
